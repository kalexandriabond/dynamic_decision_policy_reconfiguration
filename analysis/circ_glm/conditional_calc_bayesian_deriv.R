...
# predicted = predict(conditional_models$m[[1]]) # get predictions of winning model
...
for (subject in seq(nrow(conditional_models))){
  for (model in colnames(conditional_models)[-1]){
      
      conditional_models[[model]][[subject]] = calc_BIC(conditional_models[[model]][[subject]])    
      conditional_models[[model]][[subject]] = calc_prior_predictive_probability(conditional_models[[model]][[subject]])
        
  }}

# calculating the BF for all alt models against the null 

null_BFs <- vector("list", (ncol(conditional_models)-2)*4)

i = 0

for (subject in seq(nrow(conditional_models))) {
  for (model in colnames(conditional_models)[c(-1, -2)]) {
    # all except the subj_id and absolute null model
    
    i = i + 1
    
    null_BFs[[i]] = calc_BF01(conditional_models[[model]][[subject]],
                              conditional_models[[2]][[subject]],
                              conditional_models$subj_id[subject]) # alt v null model
  }
}


null_BF_objs <- vector("list", (ncol(conditional_models)-2)*4)
...
for (subject in seq(nrow(conditional_models))) {
  for (model in colnames(conditional_models)[c(-1, -2)]) {
    ...
    null_BF_objs[[i]] = calc_BF01(conditional_models[[model]][[subject]],
                              conditional_models[[2]][[subject]],
                              conditional_models$subj_id[subject]) # alt v null model
...
BFs = vector("list", (ncol(conditional_models)-2)*4)
subj_id = vector("list", (ncol(conditional_models)-2)*4)
BF_alt = vector("list", (ncol(conditional_models)-2)*4)
comparison = vector("list", (ncol(conditional_models)-2)*4)
...
cond_alt_v_null_df <- data.frame(BF=matrix(unlist(BFs), nrow=24, byrow=T),
                 subj_id=matrix(unlist(subj_id), nrow=24, byrow=T), 
                 BF_alt=matrix(unlist(BF_alt), nrow=24, byrow=T), 
                 comparison=matrix(unlist(comparison), nrow=24, byrow=T), 
...
BICs = vector("list", (ncol(conditional_models)-1)*4)
models = vector("list", (ncol(conditional_models)-1)*4)
subj_id = vector("list", (ncol(conditional_models)-1)*4)
...
for (subject in seq(nrow(conditional_models))){
  for (model in colnames(conditional_models)[-1]){
    ...
    BICs[[i]] = conditional_models[[model]][[subject]]$BIC
    models[[i]] = model
    subj_id[[i]] = conditional_models$subj_id[[subject]]
    
  }}


bic_df <- data.frame(bic=matrix(unlist(BICs), nrow=28, byrow=T),
                               subj_id=as.factor(matrix(unlist(subj_id), nrow=28, byrow=T)), 
                     model=matrix(unlist(models), nrow=28, byrow=T), 
                               stringsAsFactors=FALSE)

bic_df <- bic_df %>% group_by(subj_id) %>% mutate(null_adj_bic = bic - bic[model=='abs_null_m'])
...
prior_pred_prob = vector("list", (ncol(conditional_models)-1)*4)
models = vector("list", (ncol(conditional_models)-1)*4)
subj_id = vector("list", (ncol(conditional_models)-1)*4)
...
for (subject in seq(nrow(conditional_models))){
  for (model in colnames(conditional_models)[-1]){
    ...
    prior_pred_prob[[i]] <- conditional_models[[model]][[subject]]$prior_pred_prob
...
prior_pred_prob_df <- data.frame(prior_pred_prob=matrix(prior_pred_prob, nrow=28, byrow=T),
                               subj_id=matrix(unlist(subj_id), nrow=28, byrow=T), model=matrix(unlist(models), nrow=28, byrow=T), 
                               stringsAsFactors=FALSE)

prior_pred_prob_df$subj_id_anon <- recode_factor(prior_pred_prob_df$subj_id, `786` = 1, `787` = 2, `788` = 3, '789' = 4)


subj_id = vector("list", (nrow(conditional_models)))
posterior_prob_source_model = vector("list", (nrow(conditional_models)))
...
for (subject in unique(prior_pred_prob_df$subj_id_anon)){
  ...
  source_model_prior_pred_prob = sub_data[which(sub_data$model == 'abs_null_m'),]$prior_pred_prob
...
posterior_prob_abs_null_df <- data.frame(posterior_prob_abs_null=matrix(unlist(posterior_prob_source_model), nrow=4, byrow=T),
...
for (subject in unique(prior_pred_prob_df$subj_id_anon)){
  ...
  sub_data = subset(prior_pred_prob_df, subj_id_anon == subject)
  source_model_prior_pred_prob = sub_data[which(sub_data$model == 'time_null_m'),]$prior_pred_prob
  all_models_prior_pred_prob = sub_data$prior_pred_prob
  # 
  # print(source_model_prior_pred_prob)
  # print(all_models_prior_pred_prob)
...
posterior_prob_time_null_df <- data.frame(posterior_prob_time_null=matrix(unlist(posterior_prob_source_model), nrow=4, byrow=T),
                                     subj_id=matrix(unlist(subj_id), nrow=4, byrow=T), 
                                     stringsAsFactors=FALSE)
...
time_null_BFs <- vector("list", (ncol(conditional_models)-5)*4)

i = 0

for (subject in seq(nrow(conditional_models))) {
  for (model in colnames(conditional_models)[c(-1, -2, -3, -6, -7)]) {
    # all except the subj_id and absolute null model
    
    i = i + 1
    
    time_null_BFs[[i]] = calc_BF01(conditional_models[[model]][[subject]],
                              conditional_models[[3]][[subject]],
                              conditional_models$subj_id[subject]) # alt v null model
  }
}


time_null_BF_objs <- vector("list", (ncol(conditional_models)-5)*4)
...
for (subject in seq(nrow(conditional_models))) {
  for (model in colnames(conditional_models)[c(-1, -2, -3, -6, -7)]) {
    ...
    time_null_BF_objs[[i]] = calc_BF01(conditional_models[[model]][[subject]],
                                  conditional_models[[3]][[subject]],
                                  conditional_models$subj_id[subject]) # alt v null model
    print(paste(time_null_BF_objs[[i]]$subject, time_null_BF_objs[[i]]$comparison))
  }
}


BFs = vector("list", (ncol(conditional_models)-5)*4)
subj_id = vector("list", (ncol(conditional_models)-5)*4)
BF_alt = vector("list", (ncol(conditional_models)-5)*4)
comparison = vector("list", (ncol(conditional_models)-5)*4)

i = 0

for (obj in time_null_BF_objs) {
  
  i = i + 1
  
  subj_id[[i]] = time_null_BFs[[i]]$subject
  BFs[[i]] = time_null_BFs[[i]]$BF
  BF_alt[[i]] = time_null_BFs[[i]]$BF_alt
  comparison[[i]] = time_null_BFs[[i]]$comparison
  
}

cond_alt_v_time_null_df <- data.frame(BF=matrix(unlist(BFs), nrow=24, byrow=T),
                                 subj_id=matrix(unlist(subj_id), nrow=24, byrow=T), 
                                 BF_alt=matrix(unlist(BF_alt), nrow=24, byrow=T), 
                                 comparison=matrix(unlist(comparison), nrow=24, byrow=T), 
                                 stringsAsFactors=FALSE)
...
n_models = ncol(conditional_models)-1
subj_id = vector("list", (nrow(conditional_models))*n_models)
models = vector("list", (nrow(conditional_models))*n_models)
posterior_prob_all_models = vector("list", (nrow(conditional_models))*n_models)
...
all_models_posterior_prob_df <- data.frame(posterior_prob=matrix(unlist(posterior_prob_all_models), nrow=length(posterior_prob_all_models), byrow=T),
                                           subj_id=factor(matrix(unlist(subj_id), nrow=length(posterior_prob_all_models), byrow=T)), model=factor(matrix(unlist(models), nrow=length(posterior_prob_all_models), byrow=T)), 
                                           stringsAsFactors=FALSE)
...
ggplot(all_models_posterior_prob_df, aes(model_reordered, posterior_prob)) + geom_bar(stat = "summary", fun.y = "mean") + 
