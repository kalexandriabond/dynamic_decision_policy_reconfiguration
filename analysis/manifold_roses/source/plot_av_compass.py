...
savefig=None, conditional=False, all_subs_all_conds=True, pub_quality_savefig=None, home = os.path.expanduser('~'), id_str=None, sample_sub=None):
    '''This plots the directioanl distribution in the form of a polar histogram (a "rose").'''
...
all_conditions=False, r_constant=None, id_str=None, pub_quality_savefig=None,
 home = os.path.expanduser('~'), all_subs_all_conds=False):
    '''This plots the vector distribution on a polar reference frame (including magnitude & direction).'''

    import matplotlib.pyplot as plt

    fig_path=os.path.join(home, 'Dropbox/loki_0_0.5/loki_0.5_figures/')

    fig, main_ax = plt.subplots(n_rows, n_cols, subplot_kw=dict(polar=True))
    #  gridspec_kw={'hspace': 0.3, 'wspace': 0.4, 'top': 0.88,
    ...
    for shifted_trial, ax in zip(shifted_epoch_trials, ax_list):
        ...
        if shifted_trial == 0:
            ax.set_title(r'$\vec{{_{{-\textrm{}:\textrm{}}}}}$'.format(abs(int(shifted_trial-1)), int(shifted_trial)), fontsize=15, y=1.30)
        else:
            ax.set_title(r'$\vec{{_{{\textrm{}:\textrm{}}}}}$'.format(int(shifted_trial)-1, int(shifted_trial)), fontsize=15, y=1.30)
    ...
    if all_conditions:
        ...
        if id_str:
            fig_name = ('all_subs_all_conditions_av_manifold_polar_vector_dist' + id_str + '.png')
        else:
            fig_name = ('all_subs_all_conditions_av_manifold_polar_vector_dist.png')
...
_,_ = plot_compass(av_manifold_subset_df, savefig=True, n_rows=1, n_cols=4, all_subs_all_conds=True, plot_mean_components=True)
